...
class Belvg_Referralreward_Model_Friends extends Mage_Core_Model_Abstract{
    ...
    public function saveFriends($customer_id, $emails, $names){
        $count  = array('enable'=>0, 'disable'=>0);
     ...
     *
     * @param int Customer id
     * @param int Statuses of invited friends (self::FRIEND_NO_BRING, self::FRIEND_INVITED, self::FRIEND_BRING)
     * @return Belvg_Referralreward_Model_Friends_Collection
     */
    public function getFriendsCollection($customer_id, $status){
        $collection = $this->getCollection()
            ->addFieldToFilter('customer_id', $customer_id)
