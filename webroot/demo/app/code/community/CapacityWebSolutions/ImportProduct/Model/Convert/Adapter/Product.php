...
class CapacityWebSolutions_ImportProduct_Model_Convert_Adapter_Product extends Mage_Catalog_Model_Convert_Adapter_Product
{
    ...
    public function saveRow(array $importData)
    {
		...
		if (isset($importData['categories'])) {
            ...
            if(strpos($importData['categories'],'|') !== FALSE){
                $arrCate = explode('|',$importData['categories']);
                $categ = '';
                foreach($arrCate as $v){
                    $categ .= $this->_addCategories($v, $store).',';  
                    // Mage::log($categ);                    
                }
                 // $product->setCategoryIds("1151,1218");
                if ($categ) {
                    $product->setCategoryIds(explode(',',substr($categ,0,strlen($categ)-1)));
                    // $product->setCategoryIds("1151,1218");
                }
            }else{            
