...
//				catch (Exception $e) {}
    ...
    public function giveBonus($referralId){
        ...
        if(Mage::helper('referafriend')->getBonusType() == 1)
            $bonus = Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->getSymbol().money_format('%i', $discount->getAmount());
        else
                        ...
                        switch ($rule->getApplies()){
                            case AW_Referafriend_Model_Rule::APPLY_PER_CUSTOMER:
                                foreach ($referralIds as $referralId){
                                    ...
                                    $discountAmount[$referralId] = $discountModel->calculate($rule, $signups, $amount, $qty, $rule->getTrigCount(),$referrerId);
                                }
                                break;
